$(function(){
    ...
    function(){
        ...
        //위로 버튼 클릭하면 화면 제일위로 올라기
        $('.fa-arrow-up').click(function(){
            $('html,body').animate({'scrollTop':0});
        });  

        //menu_brn버튼으 클릭하면 #navi나옴
        $('.menu_btn').click(function(){
            $('#navi').animate({'right':0});
        });
        //fa-arrow-right버튼을 클릭하면 #navi들어감
        $('.fa-arrow-right').click(function(){
            $('#navi').animate({'right':'-100vw'});
            return false;
        });  
        //태블릿 모바일 버전 네이게이션(세로 아코디언메뉴)
        $('#navi nav > ul >li > a').click(function(){
            //만약 클릭한 메뉴에 active가 설정 되어있지 않다면, !=(비교연산자)
            if($(this).attr('class') != 'active'){
                //모든 ㅅ브메뉴 들어감
                $('#navi nav .sub').slideUp();
                //모든 메뉴에서 active제거
                $('#navi nav > ul > li > a').removeClass('active');
                //클릭한 메뉴의 서브메뉴만 나옴 next():다음형제 가리킴/find : 가리키는 형제모두 찾음
                $(this).next().slideDown();
                //클릭한 메뉴에만 active설정
